...
class BatchedPerturbationConfig:
    ...
    def is_noop(self) -> bool:
        """True when no member carries any perturbation (e.g. built via empty()).

        da3 inference addition, deliberately block-independent: lets callers skip
        the per-block ``is_perturbed(..., self.idx)`` checks, whose ``idx`` guards
        otherwise force torch.compile to specialize every transformer block
        separately (48 blocks -> recompile_limit overflow -> eager fallback).
        """
        return all(not cfg.perturbations for cfg in self.perturbations)
