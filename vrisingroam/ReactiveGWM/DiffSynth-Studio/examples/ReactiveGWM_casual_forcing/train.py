...
按 --stage 派发:
  Stage 1 (ar_tf): TF dual-block flow-match teacher forcing  ← 当前可用
  Stage 2 (cd):    Locked (等 Stage 1 review)
  Stage 3 (dmd):   Locked (等 Stage 2 review)

YAML 是单一来源 (configs/<stage>.yaml; 与 configs/default.yaml 浅合并).
环境变量 (BASE_CKPT / DATA_ROOT / OUT / STUDENT_INIT) 覆盖 yaml.
...
同节奏 spawn 非阻塞 sanity 推理 subprocess (永不阻塞训练).
"""
...
# sys.path setup:
#   _HERE             = examples/ReactiveGWM_casual_forcing/  (so `inference.*`,
#                       `modules.*` are top-level importable)
#   _REACTIVE_GWM     = examples/ReactiveGWM/                  (for `data.*`)
#   _REPO_ROOT        = DiffSynth-Studio/                       (for full
#                       `examples.ReactiveGWM_casual_forcing.*` paths)
_HERE = Path(__file__).resolve().parent
_REACTIVE_GWM = _HERE.parent / "ReactiveGWM"
_REPO_ROOT = _HERE.parent.parent
for p in (_HERE, _REACTIVE_GWM, _REPO_ROOT):
...
def _run_dmd(args: argparse.Namespace, cfg: dict[str, Any]) -> None:
    ...
    """
    from examples.ReactiveGWM_casual_forcing.modules.runner_dmd import run_dmd
...
def _run_ar_tf(args: argparse.Namespace, cfg: dict[str, Any]) -> None:
    ...
    # Use full module paths — `inference` and `modules` collide with the
    # same-named subdirs under examples/ReactiveGWM/ (CF and SFT each have
    # their own); rely on the repo-root entry in sys.path instead.
    from examples.ReactiveGWM_casual_forcing.modules.ar_tf import CFARTrainingModule
    from examples.ReactiveGWM_casual_forcing.modules.dataset import build_cf_dataset
    from examples.ReactiveGWM_casual_forcing.inference.spawn_sample import (
        spawn_sanity_sample,
    )
    ...
    for epoch_id in range(num_epochs):
        ...
        for data in tqdm(dataloader, disable=not accelerator.is_main_process):
            with accelerator.accumulate(model):
                ...
                # Save full state + spawn sanity inference on save_steps boundary
                if save_steps and step % save_steps == 0:
                    ...
                    if accelerator.is_main_process:
                        ...
                        last_sanity_proc = spawn_sanity_sample(
                            ckpt_path=ckpt_path,
                            step=step,
                            out_dir=out_path,
                            cfg=cfg,
                            config_path=args.config,
                            last_proc=last_sanity_proc,
                        )
...
def _run_cd(args: argparse.Namespace, cfg: dict[str, Any]) -> None:
    ...
    与 ``_run_ar_tf`` 同构 (单 backward / accelerate state / sanity spawn), 增量:
      ...
      - 存点除 accelerate state 外, 额外写 ``state-N/ema.safetensors`` (EMA resume) +
        ``step-N.safetensors`` (导出 EMA 权重, 供 sanity / 下阶段; step<ema_start 时为 raw);
    ...
    from examples.ReactiveGWM_casual_forcing.modules.cd import CFCDTrainingModule
    from examples.ReactiveGWM_casual_forcing.modules.dataset import build_cf_dataset
    from examples.ReactiveGWM_casual_forcing.inference.spawn_sample import (
        spawn_sanity_sample,
    )
