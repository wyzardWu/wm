...
直接复用 `diffsynth.core.UnifiedDataset` + `examples/ReactiveGWM/data/` 操作算子,
schema 与 examples/ReactiveGWM SFT 训练完全一致 (CSV metadata + video parquet action).
...
def build_cf_dataset(
    *,
    ...
    依赖 `examples/ReactiveGWM/data/` 子模块在 sys.path (由 train.py 注入).
    """
    # Lazy import — sys.path 由调用者 (train.py) 注入 examples/ReactiveGWM/
    from data.action_utils import get_action_op  # noqa: WPS433
    from data.profiles import get_profile  # noqa: WPS433
