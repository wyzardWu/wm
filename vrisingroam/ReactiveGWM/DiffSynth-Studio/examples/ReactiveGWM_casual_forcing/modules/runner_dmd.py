...
与 Stage 1/2 的单优化器 runner (写在 train.py) 不同, Stage 3 需要:
  ...
  - checkpoint: accelerate state (含 generator+critic 子模块 + optimizer + RNG)
    ...
    + step-N.safetensors (generator EMA 导出, 供 sanity / 下阶段).
...
def _prune_old_ckpts(out_path: str, keep: int) -> None:
    ...
    state-N 含分片 model+optimizer (大); step-N 是 generator 导出 (~9.4GB). samples/*.mp4
    不动 (小, 留作 early/mid/late sanity 对照). keep<=0 → 不修剪. 仅主进程调用.
    """
    import glob
    import shutil
    if keep <= 0:
...
def run_dmd(args, cfg: dict[str, Any]) -> None:
    ...
    from examples.ReactiveGWM_casual_forcing.modules.dmd import CFDMDTrainingModule
    ...
    # ---------------- save helper
    def _save(step_now: int, last_proc):
        ...
        # sanity: 4-step KV-cache 自回归 (sanity_sample_diffusion_steps=4 in stage3 yaml)
        return spawn_sanity_sample(
            ckpt_path=ckpt_path, step=step_now, out_dir=out_path, cfg=cfg,
            config_path=args.config, last_proc=last_proc,
        )
    ...
    while step < max_steps:
        ...
        if save_steps and step % save_steps == 0:
            last_sanity_proc = _save(step, last_sanity_proc)
