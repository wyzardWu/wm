...
from examples.ReactiveGWM_casual_forcing.modules.ema import (
    ema_copy_to_model,
    ema_init_shadow,
    ema_update_shadow,
)

from data.profiles import get_profile  # noqa: E402  (sys.path 由 train.py 注入)
from data.prompt_utils import resolve_prompt  # noqa: E402

# DEFAULT_NEGATIVE_PROMPT 从 pipeline 模块 import (单一来源, sanity 推理也用同一份),
# 与 CF 上游 causal_cd_framewise.yaml negative_prompt 逐字一致 (Wan 默认负面串).
...
class CFCDTrainingModule(DiffusionTrainingModule):
    ...
    def cd_export_state_dict(self, step: int) -> dict[str, torch.Tensor]:
        """导出 bare-DiT 权重 (供 sanity / 下阶段). step≥ema_start → EMA (fp32 shadow→bf16),
        否则 raw generator.

        只取 generator 的可训练参数名 (与 Stage 1 step-N.safetensors 同格式; 无 buffers,
        sanity 加载时 strict=False, buffers 由 fresh init 提供, 恒等).
        """
