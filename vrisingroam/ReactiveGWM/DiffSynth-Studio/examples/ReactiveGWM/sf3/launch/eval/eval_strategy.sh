#!/bin/bash
# SF3 strategy-axis evaluation launcher.
#
# Usage:
#   bash examples/ReactiveGWM/sf3/inference/launch/eval_strategy.sh <run_name> <step>
...
#
# Output:
#   /opt/dlami/nvme/zeqingwang/evaluation/sf3/<run_name>_step<step>/strategy/
set -e
...
OUT_DIR="/opt/dlami/nvme/zeqingwang/evaluation/sf3/${RUN_NAME}_step${STEP}/strategy"

echo "=== SF3 eval_strategy ==="
...
PYTHONUNBUFFERED=1 python examples/ReactiveGWM/sf3/extras/eval_strategy.py \
  --full_ckpt "${FULL_CKPT}" \
  --output_dir "${OUT_DIR}" \
  --reference_clip "${REFERENCE_CLIP}" \
  --gpu_ids "${GPU_IDS}" \
