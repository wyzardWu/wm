"""Rebuild SF2 cache with use_csv_prompt=False (fixed-prompt training).

Existing v5 cache (built via run_cache_8shard_v5.sh) has T5 entries for
many unique per-row NPC prompts and a manifest.config with use_csv_prompt=True.
For the no-cross-attn baseline (cross_attn frozen, fixed prompt) we want a
cache where every row's prompt_hash points to T5(SF2_FIXED_PROMPT).

Plan:
  1. Hardlink video/ + first_frame/ entire trees from source cache_dir into
     dest cache_dir (zero disk increment — same inode).
  2. Encode T5 for SF2_FIXED_PROMPT + empty string. Save to dest/t5/.
  3. Write a new manifest.json:
     ...
     - rows: same csv_index/rel_video/video_hash/first_frame_hash as source,
             but every row's prompt_hash = t5_cache_key(SF2_FIXED_PROMPT)
...
from data.profiles import SF2  # noqa: E402

# Module-level alias kept for diff readability with upstream rebuild script.
SF2_FIXED_PROMPT = SF2.fixed_prompt
...
def encode_prompt_bitexact(pipe, prompt: str) -> torch.Tensor:
    ...
    examples/sf2_final/scripts/precompute_cache.py."""
...
def main():
    p = argparse.ArgumentParser(
        description="Rebuild SF2 cache with fixed-prompt T5 (hardlinks video/ff)"
    )
    ...
    fixed_hash = t5_cache_key(SF2_FIXED_PROMPT)
    empty_hash = t5_cache_key("")
    print(f"[rebuild] SF2_FIXED_PROMPT={SF2_FIXED_PROMPT!r} -> hash {fixed_hash[:16]}...")
    print(f"[rebuild] empty hash {empty_hash[:16]}...")

    for hsh, prompt in [(fixed_hash, SF2_FIXED_PROMPT), (empty_hash, "")]:
    ...
    new_cfg["fixed_prompt"] = SF2_FIXED_PROMPT
    ...
    new_manifest = {
        ...
        "rows": new_rows,
        "prompt_table": {
            fixed_hash: SF2_FIXED_PROMPT[:60],
            empty_hash: "",
        },
