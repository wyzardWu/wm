#!/bin/bash
# Launch 8 parallel cache precompute shards on GPU 0-7 for clips_5s_cache_v5.
#
# v5 metadata: 3-cat NPC prompt with paraphrase pool (Offense/Defense/Control).
# Source: metadata_v5.csv (10000 rows after strict + verify + backfill).
#
# Rank 0 (GPU 0): T5 phase (encodes ~3000 unique paraphrase prompts) + 1/8 VAE + writes manifest.
# Rank 1-7 (GPU 1..7): VAE only, --skip_manifest_write.
# All shards write to the same cache_root; atomic writes + --skip_existing make it safe.
#
# Logs: /tmp/sf2_cache_v5_rank{0..7}.log
# PIDs: /tmp/sf2_cache_v5_pids.txt
set -e
...
run_shard() {
  ...
  local RANK=$2
  local EXTRA_FLAGS=$3
  local LOG=/tmp/sf2_cache_v5_rank${RANK}.log
  ...
  --game sf2 \
    ...
    > ${LOG} 2>&1 &
  local PID=$!
  echo "shard rank=${RANK} GPU=${GPU} PID=${PID}" | tee -a /tmp/sf2_cache_v5_pids.txt
}

# Rank 0 owns T5 phase + manifest. Start FIRST.
run_shard 0 0 ""
sleep 5
...
cat /tmp/sf2_cache_v5_pids.txt
echo ""
echo "Monitor:"
echo "  tail -f /tmp/sf2_cache_v5_rank{0..7}.log"
echo "  watch -n 30 'find ${CACHE_ROOT} -name \"*.pt\" | wc -l'"
echo ""
echo "Expected on completion: 10000 video + 10000 first_frame + ~9 t5 (paraphrase pool) = ~20000+ .pt files"
