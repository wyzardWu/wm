#!/bin/bash
# Launch 8 parallel cache precompute shards on GPU 0-7 for clips_5s_cache_v4 (random-only, ~10k clips).
#
# Reuses pre-filled v4 cache (hardlinks from v2 cache); only computes missing pieces:
#   ~4466 missing video + first_frame + ~2247 missing T5 prompts.
#
# Rank 0 (GPU 0): T5 phase (encodes missing prompts) + 1/8 of VAE + writes manifest.
...
#
# Logs: /tmp/sf2_cache_v4_rank{0..7}.log
# PIDs written to /tmp/sf2_cache_v4_pids.txt
set -e
...
CACHE_ROOT="/home/zeqingwang/zeqingwang/datasets/Final_dataset/SF2/train/clips_5s_cache_v4"
METADATA="${DATA_ROOT}/metadata_v4.csv"
...
run_shard() {
  ...
  local RANK=$2
  local EXTRA_FLAGS=$3
  local LOG=/tmp/sf2_cache_v4_rank${RANK}.log
  ...
  --game sf2 \
    ...
    > ${LOG} 2>&1 &
  local PID=$!
  echo "shard rank=${RANK} GPU=${GPU} PID=${PID}" | tee -a /tmp/sf2_cache_v4_pids.txt
}
...
cat /tmp/sf2_cache_v4_pids.txt
echo ""
echo "Monitor:"
echo "  tail -f /tmp/sf2_cache_v4_rank{0..7}.log"
echo "  watch -n 30 'find ${CACHE_ROOT} -name \"*.pt\" | wc -l'"
echo ""
echo "Expected on completion: 9999 video + 9999 first_frame + ~6615 t5 = ~26000+ .pt files"
