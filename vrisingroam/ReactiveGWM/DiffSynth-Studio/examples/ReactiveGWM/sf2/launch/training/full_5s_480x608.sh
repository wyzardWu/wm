#!/bin/bash
# Mode A: Full DiT fine-tune (all ~5B params). 8 GPU (0-7), bs=1/GPU, accum=1 -> eff bs=8.
set -e
...
OUT="/home/zeqingwang/zeqingwang/models/train/sf2/Baseline/full_5s_480x608"
RUN_NAME="full_5s_480x608"

eval "$(conda shell.bash hook 2>/dev/null)"
conda activate diffsynth

echo "=== SF2_final / Mode A: Full DiT fine-tune ==="
echo "Data:   ${DATA_ROOT}/metadata.csv"
echo "Output: ${OUT}"
echo "GPUs:   ${CUDA_VISIBLE_DEVICES}  bs=1/GPU accum=1"
echo "Start:  $(date)"

PYTHONUNBUFFERED=1 accelerate launch --num_processes=8 --multi_gpu --main_process_port 29601 \
