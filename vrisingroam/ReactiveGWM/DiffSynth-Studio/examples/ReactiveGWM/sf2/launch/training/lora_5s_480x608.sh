#!/bin/bash
# Mode C: LoRA on cross_attn + self_attn (rank=32). Backbone frozen.
set -e
...
OUT="/home/zeqingwang/zeqingwang/models/final_model/sf2/baseline/lora_cross_self_r32"

eval "$(conda shell.bash hook 2>/dev/null)"
conda activate diffsynth

echo "=== SF2_final / Mode C: LoRA cross_attn + self_attn (rank=32) ==="
echo "Output: ${OUT}"
echo "Start:  $(date)"

PYTHONUNBUFFERED=1 accelerate launch --num_processes=3 --multi_gpu --main_process_port 29604 \
