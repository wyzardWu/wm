...
METADATA="${METADATA:-${REBUTTAL_DIR}/generated/metadata_isaac_v1_smoke16.csv}"
...
OUT="${OUT:-/nfs/zeqingwang/models/train/ReactiveGWM/Rebuttal/Isaac_v1_smoke}"
PORT="${PORT:-29741}"

export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,3,4,5}"
if [[ "${CUDA_VISIBLE_DEVICES}" != "0,3,4,5" ]]; then
  echo "ERROR: Isaac smoke requires CUDA_VISIBLE_DEVICES=0,3,4,5" >&2
  exit 1
fi
