...
BUDGET=999000   # open-ended: train until user says stop
LOG=runs/v2ca_grab.log
log(){ echo "$(date '+%m-%d %H:%M') $*" >> "$LOG"; }
state_ok2(){ d=$1; [ -d "$d" ] && [ -z "$(ls $d/.tmp* 2>/dev/null)" ] && [ -f "$d/model.safetensors" ] && [ -f "$d/optimizer.bin" ]; }
...
launch_ca(){  # $1 gpus csv, $2 resume STATE dir ("" = cold start), $3 max steps
  EXTRA=""
  [ -n "$2" ] && EXTRA="--resume_state $2"
  ...
  CUDA_VISIBLE_DEVICES=$1 NPROC=$NP nohup bash scripts/train_bidirectional.sh \
    data/processed/combined_5d v2_ca_bidir_p3 \
...
MISS=0
while :; do
  N=$(ls runs/v2_ca_bidir_p3/step-*.safetensors 2>/dev/null \
      | sed 's/.*step-//; s/\.safetensors//' | sort -n | tail -1); N=${N:-0}
  [ "$N" -ge "$BUDGET" ] && { log "v2_ca_bidir budget complete ($N)"; break; }
  # state janitor: keep only the 3 newest full-state dirs (~27GB each)
  for d in $(ls -d runs/v2_ca_bidir_p3/state-* 2>/dev/null | sort -t- -k3 -n | head -n -3); do rm -rf "$d"; done
  if pgrep -u yuzhewu -f "bidirectional.trai[n]" >/dev/null; then
    ...
    if [ "$CUR_NP" -le 2 ] && [ "$N" -gt 0 ]; then
      ...
      if [ "$NF_" -ge 4 ]; then
        ...
        if [ "$NC" -ge 4 ]; then
          ...
          launch_ca "$(echo "$C" | head -4 | paste -sd, -)" "runs/v2_ca_bidir_p3/step-$N.safetensors" "$REMAIN"
          sleep 900
        fi
      fi
    fi
    ...
    if [ "$MISS" -ge 2 ]; then
      GPUS=$(wait_for_cards)
      LATEST_STATE=""
      for d in $(ls -d runs/v2_ca_bidir_p3/state-* 2>/dev/null | sort -t- -k3 -n -r); do
        state_ok2 "$d" && { LATEST_STATE=$d; break; }
      done
      if [ -n "$LATEST_STATE" ]; then
        launch_ca "$GPUS" "$LATEST_STATE" 999000
      else
        log "no complete state dir; waiting"; sleep 600
      fi
      MISS=0; sleep 900
    fi
  fi
