...
launch_ca(){  # $1 gpus csv, $2 resume ckpt ("" = cold start), $3 remaining steps
  ...
  CUDA_VISIBLE_DEVICES=$1 NPROC=$NP nohup bash scripts/train_bidirectional.sh \
    data/processed/combined_5d v2_ca_bidir_p4 \
...
MISS=0
while :; do
  N=$(ls runs/v2_ca_bidir_p4/step-*.safetensors 2>/dev/null \
  ...
  for d in $(ls -d runs/v2_ca_bidir_p4/state-* 2>/dev/null | sort -t- -k3 -n | head -n -3); do rm -rf "$d"; done
  if pgrep -u yuzhewu -f "bidirectional.trai[n]" >/dev/null; then
    ...
    if [ "$CUR_NP" -le 2 ] && [ "$N" -gt 0 ]; then
      ...
      if [ "$NF_" -ge 4 ]; then
        ...
        if [ "$NC" -ge 4 ]; then
          ...
          launch_ca "$(echo "$C" | head -4 | paste -sd, -)" "runs/v2_ca_bidir_p4/step-$N.safetensors" "$REMAIN"
          sleep 900
        fi
      fi
    fi
    ...
    if [ "$MISS" -ge 2 ]; then
      ...
      GPUS=$(wait_for_cards)
      if [ "$N" -gt 0 ]; then
        launch_ca "$GPUS" "runs/v2_ca_bidir_p4/step-$N.safetensors" "$REMAIN"
      else
        launch_ca "$GPUS" "runs/v2_ca_bidir_p3/step-54000.safetensors" "$BUDGET"  # p4 root = p3 true-30k
      fi
      MISS=0; sleep 900
    fi
  fi
